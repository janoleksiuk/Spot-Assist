...
def shutdown_handler(signum, frame):
    print("\n🛑 Interrupt signal received. Spot will sit down safely...")
    if robot_command_client:
        stop_moving(robot_command_client)
        try:
            robot_command_client.robot_command(RobotCommandBuilder.claw_gripper_open_command())
            print("🖐️ Opening the gripper and waiting for the person to take the object...")
            time.sleep(2.0)
            ...
            print(f"⚠️ Error occurred during interrupt exit: {e}")
    else:
        print("⚠️ robot_command_client is not initialized. Unable to send sit command.")
    cv2.destroyAllWindows()
    sys.exit(0)


def detect_objects(image_client, model, source_name='hand_color_image'):
...
def start_rotating(robot_command_client, yaw_speed=ROTATE_SPEED, duration_sec=2.0):
    cmd = RobotCommandBuilder.synchro_velocity_command(0, 0, yaw_speed)
...
def stop_moving(robot_command_client):
    try:
        cmd = RobotCommandBuilder.synchro_velocity_command(0, 0, 0)
        robot_command = RobotCommandBuilder.build_synchro_command(cmd)
        robot_command_client.robot_command(robot_command, end_time_secs=time.time() + 1.0)
    except Exception as e:
        print(f"⚠️ Failed to stop movement：{e}")
...
def main():
    ...
    try:
        ...
        with LeaseKeepAlive(lease_client, must_acquire=True, return_at_exit=True):
            ...
            person_found = search_for_person(robot_command_client, image_client, model)

            if person_found:
                deliver_object_to_person(robot_command_client, image_client, model)
            else:
                print("❌ Delivery failed: No person detected...")
